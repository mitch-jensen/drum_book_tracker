...
class Section(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["book", "order"], name="unique_section_order"
            )
        ]
...
class Exercise(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["section", "exercise_number"], name="unique_exercise_number"
            )
        ]
...
class PracticeLog(models.Model):
    ...
    exercise = models.ForeignKey(
        Exercise, on_delete=models.CASCADE, related_name="practice_logs"
    )
    practiced_on = models.DateField(auto_now_add=True)
    tempo = models.IntegerField()
    notes = models.TextField(blank=True)
    difficulty = models.CharField(
        max_length=10, choices=Difficulty.choices, default=Difficulty.NOT_RATED
    )
